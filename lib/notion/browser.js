 ...
 * Falls back to the Notion ledger snapshot when the API routes are missing.
 */
(function () {
  ...
  var MOCK_HINT = 'Demo seed from Notion snapshot. Wire NOTION_TOKEN on the server (or Vercel) to load live Notion.';
  ...
  function loadNotionExpenses(force) {
    ...
    expenseInflight = fetchJson('api/expenses')
      ...
      .catch(function (err) {
        expenseInflight = null;
        var snap = snapshotFromMock(MOCK_HINT, err && err.message);
        expenseCache = snap;
        return snap;
      });
