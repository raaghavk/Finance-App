 ...
 *
 * Do not set height: -webkit-fill-available on html/body/#root — that plus
 * overflow:hidden collapses standalone Safari to a white screen.
 */
'use strict';
...
function zenithLockViewport(win) {
  const target = win || (typeof window !== 'undefined' ? window : null);
  if (!target || !target.document || typeof target.document.getElementById !== 'function') return 0;
  const h = Number(target.innerHeight) || 0;
  if (h < 100) return 0;
  ...
  if (target.document.body && target.document.body.style) {
    target.document.body.style.minHeight = h + 'px';
  }
  return h;
}
...
function zenithApplyIosSafeArea(win) {
  ...
  if (root && root.style) {
    ...
    if (pad.top > 0) {
      root.style.setProperty('--zenith-pad-top', (pad.top + 14) + 'px');
    }
    if (pad.bottom > 0) {
      root.style.setProperty('--zenith-pad-bottom', (108 + pad.bottom) + 'px');
    }
    root.dataset.zenithStandalone = pad.standalone ? '1' : '0';
  }
  zenithLockViewport(target);
  return pad;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { zenithIosStandalone, zenithSafeAreaFallback, zenithApplyIosSafeArea, zenithLockViewport };
}
if (typeof window !== 'undefined') {
  ...
  window.zenithLockViewport = zenithLockViewport;
